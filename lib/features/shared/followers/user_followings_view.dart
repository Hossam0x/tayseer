...
import 'package:tayseer/features/shared/followers/data/cubit/user_followings_cubit.dart';
import 'package:tayseer/features/shared/followers/data/cubit/user_followings_state.dart';
...
class UserFollowingsView extends StatefulWidget {
  final String userId;

  const UserFollowingsView({super.key, required this.userId});

  @override
  State<UserFollowingsView> createState() => _UserFollowingsViewState();
}

class _UserFollowingsViewState extends State<UserFollowingsView> {
  late ScrollController _scrollController;
  final TextEditingController _searchController = TextEditingController();
  late UserFollowingsCubit _cubit;
  ...
  void initState() {
    super.initState();
    _cubit = UserFollowingsCubit(
      repository: getIt<UserFollowingsRepository>(),
  ...
  void _onScroll() {
    if (_scrollController.position.pixels ==
      ...
      _cubit.fetchFollowings(loadMore: true);
    }
  }
          ...
          child: SafeArea(
            child: Column(
              children: [
                ...
                // Following List
                Expanded(
                  child: BlocBuilder<UserFollowingsCubit, UserFollowingsState>(
  ...
  Widget _buildSearchField() {
    return BlocBuilder<UserFollowingsCubit, UserFollowingsState>(
  ...
  Widget _buildContent(BuildContext context, UserFollowingsState state) {
    // حالة البحث: نعرض loading مختلف
    if (state.isSearching && state.followings.isEmpty) {
      return _buildSearchLoading();
    }

    if (state.isLoading && state.followings.isEmpty) {
      return _buildSkeletonList();
    }

    if (state.errorMessage != null && state.followings.isEmpty) {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemCount: state.followings.length + (state.hasMore ? 1 : 0),
        itemBuilder: (context, index) {
          ...
          return FollowerItem(
            follower: following,
            onToggleFollow: () {
              _cubit.toggleFollow(following.id, index);
            },
          );
        },
      ),
  ...
  Widget _buildLoadingMoreWidget(UserFollowingsState state) {
